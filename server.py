...
# Pour cree un serveur avec Socket
server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
server_socket.bind(('localhost', 33066))
...
def connection_requests():
    ...
    while True:
        ...
        print(f"Les connexions de {address} ont été établi")
        print(len(clients_connected))
        if len(clients_connected) == 3:
        ...
        try:
            ...
            print(f"{address} Coupé")
            client_socket.close()
            continue

        print(f"{address} s'est identifié comme {client_name}")
        ...
        if client_socket.recv(1024).decode() == 'image_received':
            ...
            for client in clients_connected:
                if client != client_socket:
                    ...
                    data = pickle.dumps(
                        {'message': f"{clients_connected[client_socket][0]} rejoint le chat", 'extension': image_extension,
...
def receive_data(client_socket):
    while True:
        try:
            ...
            print(f"{clients_connected[client_socket][0]} coupé")

            for client in clients_connected:
                if client != client_socket:
                    client.send('notification'.encode())

                    data = pickle.dumps({'message': f"{clients_connected[client_socket][0]} quitté le chat",
            ...
            print(f"{clients_connected[client_socket][0]} déconnecté de la façon inattendue.")
