...
class FirstScreen(tk.Tk):
    ...
    def __init__(self):
        ...
        head = tk.Label(self.first_frame, text="S'inscrire", font="lucida 17 bold",fg="white", bg="blue")
        head.place(relwidth=1, y=24)

        self.profile_label = tk.Label(self.first_frame, bg="Silver")
        ...
        upload_button.place(x=350, y=220,width=150)

        self.username = tk.Label(self.first_frame, text="Nom de l'utilisateur", font="lucida 12 bold", bg="Silver")
        self.username.place(x=40, y=150)

        self.username_entry = tk.Entry(self.first_frame,  font="lucida 12 bold", width=12,
                                       highlightcolor="blue", highlightthickness=1.5)
        self.username_entry.place(x=195, y=150)

        self.username_entry.focus_set()

        submit_button = tk.Button(self.first_frame, text="Relier", font="lucida 12 bold", padx=30, cursor="hand2",
                                  command=self.process_data, bg="Purple",fg="white",  bd=2)
    ...
    def add_photo(self):
        ...
        if self.image_path:
            ...
            user_image.save('redimensinné'+image_name)
            user_image.close()

            self.image_path = 'redimensinné'+image_name
            user_image = Image.open(self.image_path)
    ...
    def process_data(self):
        if self.username_entry.get():
            ...
            try:
                client_socket.connect(('localhost', 33066))
                status = client_socket.recv(1024).decode()
                if status == 'not_allowed':
                    client_socket.close()
                    messagebox.showinfo(title="Impossible de se connecter !", message='Désolé, le serveur est complélement occupé.'
                                                                         'Réessayez plus tard')
                    return

            except ConnectionRefusedError:
                messagebox.showinfo(title="Impossible de se connecter !", message="Le serveur est hors ligne , Réessayez plus tard.")
                print("Le serveur est hors ligne , Réessayez plus tard.")
                return
...
class ChatScreen(tk.Canvas):
    ...
    def __init__(self, parent, first_frame, client_socket, clients_connected, user_id):
        ...
        y_co = int((screen_height / 2) - (750 / 2))
        self.parent.geometry(f"555x680+{x_co}+{y_co}")
        ...
        self.create_text(440, 125, text="En ligne", font="lucida 12 bold", fill="#40C961")

        tk.Label(self, text="   ", font="lucida 15 bold", bg="white").place(x=4, y=30)

        tk.Label(self, text="Group Chat", font="lucida 15 bold", padx=20, fg="blue",
                 bg="white", anchor="w", justify="left").place(x=88, y=29, relwidth=1)

        self.create_image(60, 40, image=group_photo)

        container = tk.Frame(self)
        container.place(x=40, y=120, width=350, height=500)
        ...
        send_button = tk.Button(self, text="Send", fg="blue", font="lucida 11 bold", bg="white", padx=10,
                                 bd=2, command=self.sent_message_format)
        send_button.place(x=325, y=640)

        self.entry = tk.Text(self, font="lucida 10 bold", width=38, height=2,
                             highlightcolor="blue", highlightthickness=1)
        self.entry.place(x=40, y=635)
        ...
        m_label = tk.Label(m_frame, wraplength=250, text=f"Bonne conversation {self.parent.user}",
        ...
        #appelation du methode client qui en ligne
        self.clients_online([])
    ...
    def receive_data(self):
        while True:
            try:
                ...
                print("Vous avez déconnecté ...")
                self.client_socket.close()
                break
            except ConnectionResetError:
                messagebox.showinfo(title='Pas de Connection !', message="Serveur hors ligne..Essayez de connecter plus tard")
                self.client_socket.close()
                self.first_screen()
                break

# Avertissement si tu veut deconnecter
    def on_closing(self):
        if self.window == 'ChatScreen':
            res = messagebox.askyesno(title='Avertissement !',message="Voulez-vous vraiment vous déconnecter ?")
            if res:
                import os
...
# Pour recoi un message
    def received_message_format(self, data):
        ...
        m_frame = tk.Frame(self.scrollable_frame, bg="white")

        m_frame.columnconfigure(1, weight=1)

        t_label = tk.Label(m_frame, bg="white",fg="black", text=datetime.now().strftime('%H:%M'), font="lucida 7 bold",
        ...
        i_label = tk.Label(m_frame, bg="white", image=im)
        i_label.image = im
...
# Pour envoyer  un message
    def sent_message_format(self, event=None):
        ...
        if message:
            ...
            t_label = tk.Label(m_frame, bg="white", fg="#595656", text=datetime.now().strftime('%H:%M'),
            ...
            i_label = tk.Label(m_frame, bg="white", image=self.user_image)
    ...
    def notification_format(self, data):
        ...
        t_label = tk.Label(m_frame, fg="#595656", bg="white", text=datetime.now().strftime('%H:%M'),
                           font="lucida 9 bold")
        t_label.pack()
...
# client qui en ligne maintenent
    def clients_online(self, new_added):
        if not new_added:
            ...
            for user_id in self.clients_connected:
                ...
                b = tk.Label(self, image=user, text=name, compound="left",fg="white", bg="#008080", font="lucida 10 bold", padx=15)
                b.image = user
                self.clients_online_labels[user_id] = (b, self.y)

                b.place(x=390, y=self.y)
                self.y += 60
            ...
            b = tk.Label(self, image=user, text=name, compound="left", fg="white", bg="#008080",
